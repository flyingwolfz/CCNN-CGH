...
class proposednet(nn.Module):
    ...
    def forward(self, x):
        ...
        quan = tfft.fftshift(tfft.fftn(init_complex, dim=(-2, -1), norm='ortho'), (-2, -1))
        quan = quan * H2
        slmfield = tfft.ifftn(tfft.ifftshift(quan, (-2, -1)), dim=(-2, -1), norm='ortho')
...
with torch.no_grad():
    for kk in range(rangege):
        ...
        quan = tfft.fftshift(tfft.fftn(gray, dim=(-2, -1), norm='ortho'), (-2, -1))
        quan2 = quan * H
        final = tfft.ifftn(tfft.ifftshift(quan2, (-2, -1)), dim=(-2, -1), norm='ortho')
        final = torch.abs(final)
        final=final.cpu().numpy()
        target_amp=target_amp.cpu().numpy()
        psnrr = psnr(final, target_amp)
        ssimm=ssim(final, target_amp)
